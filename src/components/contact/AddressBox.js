...
function AddressBox({ item }) {
  return (
    <div className="w-full h-auto max-sm:bg-gradient-to-tl max-sm:to-base1 max-sm:from-base2 sm:bg-[#dceafb] p-[20px]  4xs:p-[25px] sm:p-[15px_20px] lg:p-[30px_40px] 3xl:p-[48px_60px] rounded-[15px] 2xl:rounded-[20px] 3xl:rounded-[24px] overflow-hidden relative z-0">
      <Image src={"/images/contactbx-delmt.png"} alt="contactbx-delmt" fill className="object-cover scale-[1.05] pointer-events-none" />
      ...
      {item.phone && <LinkBox href={`tel:${item.phone}`} src="/images/icon-call.svg" title={item.phone} alt="call" items={item.phone?.split(",").map(s => s.trim())} />}
      {item.email && <LinkBox href={`mailto:${item.email?.trim()}`} src="/images/icon-ft-email.svg" title={item.email?.trim()} alt="mail" items={item.email?.split(",").map(s => s.trim())} />}
    </div>
  );
}
