...
function LinkBox({ href, src, title, alt, items }) {
  const prefix = href?.startsWith("mailto:") ? "mailto:" : "tel:";
  ...
  return (
    <div className="group w-full flex items-center my-[5px] 3xl:my-[10px]">
      ...
      <div className="text-[14px] sm:text-[14px] lg:text-[16px] 2xl:text-[18px] 3xl:text-[20px] font-bold text-white sm:text-[#1b1b1b] ml-[6px] lg:ml-[10px] 3xl:ml-[14px]">
        {Array.isArray(items) ? (
          items?.map((item, idx) => (
            <span key={idx}>
              <a href={`${prefix}${item}`} aria-label={item} className="hover:text-base2 transition-color duration-300">
                {item}
              </a>
              {idx < items.length - 1 && ", "}
            </span>
