import Image from "next/image";
import parse, { domToReact } from "html-react-parser";

const options = {
  replace: (node) => {
    if (node.name === "li") {
      return (
        <li className="2xl:text-[20px] xl:text-[15px] lg:text-[14px] text-[13px] text-[#323232] relative sm:pl-[20px] before:content-[''] before:absolute before:top-[6px] 2xl:before:top-[10px] before:left-0 before:rounded-full before:bg-base1 before:w-[6px] 2xl:before:w-[8px] before:h-[6px] 2xl:before:h-[8px] sm:before:block before:hidden sm:mb-[3px] mb-[5px">
          {domToReact(node.children, options)}
        </li>
      );
    }
    // Optional: strip <ul> class and use your own outside
    if (node.name === "ul") {
      return <ul className="list-none">{domToReact(node.children, options)}</ul>;
    }
  },
};

export default function WhyLAP({ title, description, image, alt }) {
  return (
    <section className="w-full pb-[30px] xl:pb-[50px] sm:rounded-[0] rounded-[20px] overflow-hidden sm:bg-none bg-[linear-gradient(156deg,_rgba(23,71,158,0.20)_6.47%,_rgba(198,59,59,0.20)_91.2%)] ">
      <div className="container">
        <div className="w-full sm:rounded-[20px] overflow-hidden sm:bg-gradient-to-br sm:from-[rgba(23,71,158,0.20)] sm:to-[rgba(198,59,59,0.20)] pt-[30px] sm:py-[40px] xl:py-[60px] 2xl:py-[80px] 3xl:py-[100px] sm:px-[40px]">
          <div className="flex flex-wrap md:-mx-[15px] lg:-mx-[15px] xl:-mx-[20px] 2xl:-mx-[30px]">
            <div className="w-full mb-[15px] lg:mb-0 md:w-[45%] md:px-[15px] lg:px-[15px] xl:px-[20px] 2xl:px-[30px] sm:block hidden">
              <div className="flex flex-wrap h-full">
                <div className="w-full">
                  <div className="group w-full h-full 2xl:rounded-[36px] xl:rounded-[30px] md:rounded-[30px] rounded-[18px] overflow-hidden aspect-730/540">
                    <Image
                      src={image ? `${process.env.NEXT_PUBLIC_BACKEND_URL}${image}` : "/images/whyLAP.jpg"}
                      alt={alt ? alt : "lifeintelImg"}
                      width={730}
                      height={540}
                      className="w-full h-full object-cover transition-transform duration-600 group-hover:scale-[1.05]"
                    />
                  </div>
                </div>
              </div>
            </div>
            <div className="w-full md:w-[55%] md:px-[15px] lg:px-[15px] xl:px-[20px] 2xl:px-[30px]">
              <h2 className="text-title1 text-[#5e5959bf] mb-[15px] 2xl:mb-[20px] [&>span]:text-base2 [&>span]:font-bold ">{parse(title)}</h2>
              {/* <ul>
                <li className="2xl:text-[20px] xl:text-[15px] lg:text-[14px] text-[13px] text-[#323232] relative sm:pl-[20px] before:content-[''] before:absolute before:top-[6px] 2xl:before:top-[10px] before:left-0 before:rounded-full before:bg-base1 before:w-[6px] 2xl:before:w-[8px] before:h-[6px] 2xl:before:h-[8px] sm:before:block before:hidden sm:mb-[3px] mb-[5px]">
                  Collateral based and non-collateral based structured loans.
                </li>
                <li className="2xl:text-[20px] xl:text-[15px] lg:text-[14px] text-[13px] text-[#323232] relative sm:pl-[20px] before:content-[''] before:absolute before:top-[6px] 2xl:before:top-[10px] before:left-0 before:rounded-full before:bg-base1 before:w-[6px] 2xl:before:w-[8px] before:h-[6px] 2xl:before:h-[8px] sm:before:block before:hidden sm:mb-[3px] mb-[5px]">
                  Minimum documentation – for customers with high cash flows but low or nil income proof and inadequate banking documents.
                </li>
                <li className="2xl:text-[20px] xl:text-[15px] lg:text-[14px] text-[13px] text-[#323232] relative sm:pl-[20px] before:content-[''] before:absolute before:top-[6px] 2xl:before:top-[10px] before:left-0 before:rounded-full before:bg-base1 before:w-[6px] 2xl:before:w-[8px] before:h-[6px] 2xl:before:h-[8px] sm:before:block before:hidden sm:mb-[3px] mb-[5px]">
                  Income Tax Returns not mandatory.
                </li>
                <li className="2xl:text-[20px] xl:text-[15px] lg:text-[14px] text-[13px] text-[#323232] relative sm:pl-[20px] before:content-[''] before:absolute before:top-[6px] 2xl:before:top-[10px] before:left-0 before:rounded-full before:bg-base1 before:w-[6px] 2xl:before:w-[8px] before:h-[6px] 2xl:before:h-[8px] sm:before:block before:hidden sm:mb-[3px] mb-[5px]">
                  Income assessed through direct visits and real time analysis
                </li>
                <li className="2xl:text-[20px] xl:text-[15px] lg:text-[14px] text-[13px] text-[#323232] relative sm:pl-[20px] before:content-[''] before:absolute before:top-[6px] 2xl:before:top-[10px] before:left-0 before:rounded-full before:bg-base1 before:w-[6px] 2xl:before:w-[8px] before:h-[6px] 2xl:before:h-[8px] sm:before:block before:hidden sm:mb-[3px] mb-[5px]">
                  Add on services including wide range of loan schemes from Indel Money at privileged rates to ongoing LAP clients.
                </li>
                <li className="2xl:text-[20px] xl:text-[15px] lg:text-[14px] text-[13px] text-[#323232] relative sm:pl-[20px] before:content-[''] before:absolute before:top-[6px] 2xl:before:top-[10px] before:left-0 before:rounded-full before:bg-base1 before:w-[6px] 2xl:before:w-[8px] before:h-[6px] 2xl:before:h-[8px] sm:before:block before:hidden sm:mb-[3px] mb-[5px]">
                  Complete transparency and zero hidden charges.
                </li>
                <li className="2xl:text-[20px] xl:text-[15px] lg:text-[14px] text-[13px] text-[#323232] relative sm:pl-[20px] before:content-[''] before:absolute before:top-[6px] 2xl:before:top-[10px] before:left-0 before:rounded-full before:bg-base1 before:w-[6px] 2xl:before:w-[8px] before:h-[6px] 2xl:before:h-[8px] sm:before:block before:hidden sm:mb-[3px] mb-[5px]">
                  Maximum funding up to 60% on the collateral for secured loans
                </li>
                <li className="2xl:text-[20px] xl:text-[15px] lg:text-[14px] text-[13px] text-[#323232] relative sm:pl-[20px] before:content-[''] before:absolute before:top-[6px] 2xl:before:top-[10px] before:left-0 before:rounded-full before:bg-base1 before:w-[6px] 2xl:before:w-[8px] before:h-[6px] 2xl:before:h-[8px] sm:before:block before:hidden sm:mb-[3px] mb-[5px]">
                  Maximum FOIR of 60%.
                </li>
                <li className="2xl:text-[20px] xl:text-[15px] lg:text-[14px] text-[13px] text-[#323232] relative sm:pl-[20px] before:content-[''] before:absolute before:top-[6px] 2xl:before:top-[10px] before:left-0 before:rounded-full before:bg-base1 before:w-[6px] 2xl:before:w-[8px] before:h-[6px] 2xl:before:h-[8px] sm:before:block before:hidden">
                  Supply chain finance options.
                </li>
              </ul> */}
              {parse(description, options)}
              {/* {parse(description)} */}
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
