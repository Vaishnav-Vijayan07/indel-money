...
const IndelMoneyCarsGallery = ({ sliderItems = [] }) => {
  return (
    <div className="relative w-[300px] md:w-[620px] lg:w-[820px] xl:w-[1000px] 2xl:w-[1300px] mx-auto">
      <Swiper
          ...
          <SwiperSlide key={index} className="">
            <div className="relative z-0 w-[90%] h-[380px] md:h-[400px] 2xl:h-[690px] xl:h-[480px] mx-auto overflow-hidden rounded-[20px] md:rounded-[35px]">
              <div className="group w-full h-full overflow-hidden">
                <Image
                  src={slide?.image ? `${process.env.NEXT_PUBLIC_BACKEND_URL}${slide.image}` : "/images/indel-money-cars-1.jpg"}
                  alt={slide?.image_alt ? slide.image_alt : "news-1"}
                  width={1230}
                  height={690}
              ...
              <div className="absolute z-1 bottom-0 left-0 block w-full bg-gradient-to-t from-black/80 to-transparent text-white rounded-[20px] xl:rounded-[35px] 2xl:p-[0_0_55px_65px] xl:p-[0_0_40px_40px] p-[0_0_30px_30px]">
                <h2 className="2xl:text-[36px] xl:text-[24px] md:text-[22px] text-[16px] leading-[1.3] font-bold line-clamp-4 2xl:max-w-[874px] xl:max-w-[720px] md:max-w-[640px] xl:mb-[15px] mb-[10px] text-white">
                  {slide?.description ? renderHtml(slide.description) : ""}
                </h2>
                {slide?.event_date && (
