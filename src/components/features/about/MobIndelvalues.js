    ...
    <section className="w-full py-[0_30px]">
      <div className="container">
        <div className="flex flex-wrap -mx-[6px]">
          {data.map((item) => (
            <div key={item.id} className="w-1/3 p-[6px]">
              <div className="w-full h-full bg-white rounded-[10px] overflow-hidden shadow-[0_0_50px_0_rgba(0,0,0,0.10)] block p-[10px]">
                <div>
                  <Image
                    src={item.icon}
                    alt={item.title}
                    width={40}
                    height={40}
                    className="aspect-square object-contain mx-auto my-[10px]"
                  />
                  <div className="text-[13px] font-medium leading-[1.2] text-center text-base1 mb-[10px]">
                    {item.title}
                  </div>
                </div>
              </div>
