        ...
        <div className="flex flex-wrap items-center m-[-8px]">
                ...
                {lifeImages?.map((img, index) => (
                  <SwiperSlide key={index} className="flex justify-center">
                    {({ isActive, isVisible }) => {
                      ...
                      return (
                        <div
                          className="w-[300px] h-[350px] m-auto rounded-lg shadow-xl overflow-hidden relative z-0"
                          style={{
                            transform: isActive
          ...
          <div className=" w-full xl:w-[calc(100%-600px)] 2xl:w-[calc(100%-750px)] max-xl:pt-[50px] flex items-center p-[8px]">
            <div className="w-full lg:pl-[60px]">
              <div className="text-title1 text-[#5e5959bf] font-medium xl:mb-[20px] mb-[10px] [&>span]:text-base2 [&>span]:font-bold">{renderHtml(title)}</div>
              {description ? renderHtml(description) : ""}

              <Link
                href={buttonLink || "#"}
                className="group btn btn-base1 relative z-0 fle gap-2 items-center justify-between mt-[15px] lg:mt-[30px] w-fit min-w-[150px] 2xl:min-w-[200px] pr-3 pl-5 h-[45px] lg:h-[40px] 2xl:h-[50px] 3xl:h-[60px] rounded-full bg-base1 text-white font-bold transition-all duration-300 overflow-hidden shadow-lg hover:bg-base2"
              >
                <span className="relative z-10 transition-transform duration-300 group-hover:-translate-x-[-15px]">{buttonText}</span>
                <div className="relative z-10 flex items-center justify-center w-[30px] h-[30px] lg:w-[30px] lg:h-[30px] xl:w-[40px] 2xl:h-[40px] 3xl:w-[48px] 3xl:h-[48px] bg-base2 rounded-full text-red-500 transition-all duration-300  group-hover:translate-x-2 group-hover:bg-base1 group-hover:text-white">
