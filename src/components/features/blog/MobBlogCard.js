...
export default function MobBlogCard({ item, className }) {
    return (
        <Link href={`/blog/${item?.slug}`}  className={`${className ? className : ""} group w-full h-full flex gap-[10px] p-[10px] bg-white rounded-[10px] overflow-hidden`}>
            ...
            <div className="w-[calc(100%-95px)] 3xs:w-[calc(100%-113px)] py-1">
                {item?.createdAt &&
                    <div className="text-[12px] lg:text-[12px] 2xl:text-[14px] 3xl:text-[16px] text-[#505050] line-clamp-1 mb-0.5 3xl:mb-1 transition-transform duration-300 group-hover:text-base2">{format(new Date(item?.createdAt), "d MMMM yyyy")}</div>
                }
