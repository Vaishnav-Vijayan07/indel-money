...
function MapController({ selectedBranch }) {
  ...
  useEffect(() => {
    ...
    if (selectedBranch?.latitude && selectedBranch?.longitude) {
      map.setView([selectedBranch?.longitude, selectedBranch?.latitude], 13);
    }
...
export default function BranchLocationMap({ branchLocations, selectedBranch, setSelectedBranch }) {
            ...
            >
              <div className="flex items-center justify-between mb-[15px] 2xl:mb-[20px] 3xl:mb-[30px]">
                ...
                >
                  ...
                  <svg
                    className={`w-[12px] lg:w-[14px] 2xl:w-[16px] aspect-square ${selectedBranch?.id === branch?.id ? "fill-white" : ""
                      }`}
                    fill="#F30000"
                    viewBox="0 0 16 16"
                  >
