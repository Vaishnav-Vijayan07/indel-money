...
const branchLocations = [
  {
    id: 1,
    name: "Vazhakkala",
    address: "1st Floor Above Anns Bakery, Chembumukku PO Vazhakkala, Ernakulam, Kerala, 682030",
    phone: "9072588911",
...
const CustomPopup = ({ branch }) => {
  return (
    <div className="popup-content w-64">
      <h3 className="font-bold text-[18px] text-[#1B1B1B] mb-[20px]">{branch.name}</h3>
      {/* Popup content remains unchanged */}
      <div className="flex items-start mb-[15px] flex-wrap">
        <svg className="w-[16px] h-[16px] mt-[5px]" viewBox="0 0 12 16" fill="#F30000">
        ...
        <span className="w-[calc(100%-16px)] pl-[10px] text-[16px] font-normal text-[#1B1B1B] m-0">{branch.address}</span>
      </div>
      <div className="flex items-center mb-[15px] flex-wrap">
        <svg className="w-[16px] h-[16px]" viewBox="0 0 16 16" fill="#F30000">
        ...
        <span className="w-[calc(100%-16px)] pl-[10px] text-[16px] font-normal text-[#1B1B1B]">{branch.phone}</span>
      </div>
      <div className="flex items-center mb-[15px] flex-wrap">
        <svg className="w-[16px] h-[16px]" viewBox="0 0 16 16" fill="#F30000">
        ...
        <span className="w-[calc(100%-16px)] pl-[10px] text-[16px] font-normal text-[#1B1B1B]">{branch.email}</span>
      </div>
      {branch.contactNumber && (
        <div className="flex items-center flex-wrap">
          <svg className="w-[16px] h-[15px]" viewBox="0 0 16 16" fill="#F30000">
          ...
          <span className="w-[calc(100%-16px)] pl-[10px] text-[16px] font-normal text-[#1B1B1B]">{branch.contactNumber}</span>
        </div>
      )}
      <div className="mt-[20px] pt-[15px] border-t-[1px] border-[#E5E5E5] border-dashed">
        <a
          href={`https://www.google.com/maps/search/?api=1&query=${branch.coordinates[0]},${branch.coordinates[1]}`}
          target="_blank"
          className="block text-center bg-[#17479e] px-3 py-2 rounded hover:bg-[#F30000] transition-colors"
        >
          <div className="flex items-center justify-center">
            <svg className="w-[16px] h-[16px] mr-2" viewBox="0 0 16 16" fill="#ffffff">
            ...
            <span className="text-[#fff]">Get Directions</span>
          </div>
        </a>
      </div>
    </div>
  ...
  return (
    <div className="flex w-full h-[510px] 2xl:h-[670px] relative z-[1] rounded-2xl overflow-hidden">
      ...
      <div className="w-[430px] lg:w-[320px] xl:w-[360px] 2xl:w-[430px] bg-[#17479E] text-white">
        <h2 className="text-[22px] font-bold pt-[15px] pb-[15px] pl-[30px] pr-[30px] border-solid border border-[rgba(255,255,255,0.80)]">
        ...
        <div className="overflow-y-auto max-h-[calc(100%-65px)]">
          {branchLocations.map((branch) => (
            <div
              key={branch.id}
              className={`px-[15px] py-[25px] cursor-pointer loclist ${
                selectedBranch.id === branch.id ? "bg-[#f30000] active" : "hover:bg-blue-700"
              }`}
              onClick={() => handleBranchClick(branch)}
            >
              <div className="flex items-center justify-between mb-[30px]">
                <h3 className="font-bold text-[18px] text-[#fff]">{branch.name}</h3>
                <a
                  href={`https://www.google.com/maps/search/?api=1&query=${branch.coordinates[0]},${branch.coordinates[1]}`}
                  target="_blank"
                ...
                >
                  ...
                  <svg
                    className={`w-[16px] h-[16px] ${selectedBranch.id === branch.id ? "fill-white" : ""}`}
                    fill="#F30000"
                    viewBox="0 0 16 16"
                  >
              ...
              <div className="flex items-start mb-[15px] flex-wrap">
                <svg
                  className={`w-[16px] h-[16px] mt-[5px] ${selectedBranch.id === branch.id ? "fill-white" : ""}`}
                  viewBox="0 0 12 16"
                  fill="#F30000"
                >
                  <path d="M5.99772 0C2.80256 0 0.203125 2.59944 0.203125 5.79456C0.203125 9.75981 5.38872 15.581 5.6095 15.8269C5.81687 16.0579 6.17894 16.0575 6.38594 15.8269C6.60672 15.581 11.7923 9.75981 11.7923 5.79456C11.7922 2.59944 9.19284 0 5.99772 0ZM5.99772 8.70997C4.39016 8.70997 3.08234 7.40213 3.08234 5.79456C3.08234 4.187 4.39019 2.87919 5.99772 2.87919C7.60525 2.87919 8.91306 4.18703 8.91306 5.79459C8.91306 7.40216 7.60525 8.70997 5.99772 8.70997Z" />
                </svg>
                <span className="w-[calc(100%-16px)] pl-[10px] text-[16px] font-normal text-[#fff]">{branch.address}</span>
              </div>
              <div className="flex items-center mb-[15px] flex-wrap">
                <svg
                  className={`w-[16px] h-[16px] ${selectedBranch.id === branch.id ? "fill-white" : ""}`}
                ...
                <span className="w-[calc(100%-16px)] pl-[10px] text-[16px] font-normal text-[#fff]">{branch.phone}</span>
              </div>
              <div className="flex items-center mb-[15px] flex-wrap">
                <svg
                  className={`w-[16px] h-[16px] ${selectedBranch.id === branch.id ? "fill-white" : ""}`}
                ...
                <span
                  className="w-[calc(100%-16px)] pl-[10px] text-[16
              ...
              <div className="flex items-center flex-wrap">
                <svg
                  className={`w-[16px] h-[15px] ${selectedBranch.id === branch.id ? "fill-white" : ""}`}
                  viewBox="0 0 16 16"
                  fill="#F30000"
                >
                  <path d="M11.8855 5.13086L11.853 5.59086C11.8451 5.70016 11.8708 5.80926 11.9266 5.90354C11.9825 5.99782 12.0658 6.07277 12.1655 6.11836C13.058 6.52586 15.4155 7.41836 15.6805 5.58586L11.8855 5.13086Z" />
                ...
                <span className="w-[calc(100%-16px)] pl-[10px] text-[16px] font-normal text-[#fff]">{branch.contactNumber}</span>
              </div>
            </div>
          ))}
        </div>
